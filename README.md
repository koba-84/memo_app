...
本アプリケーションでは **日本語 / 英語の切り替え** に対応しています。

- **ライブラリ**: Flask-Babel
- **仕組み**
  - UIテキストを `_("...")` で外部化し、翻訳辞書 (`.po` / `.mo` ファイル) で管理
  - ユーザーがドロップダウンメニューから言語を選択可能
  - 選択結果を Cookie に保存し、リロード後も反映
- **対応範囲**
  - ナビゲーションバー、ボタン、フォームプレースホルダー、メッセージなどの UI テキスト
  - 日本語 / 英語の両方で利用可能

## Accessibility
- axe DevTools (WCAG 2.1 AA) で検証 → 自動検証の結果は問題 0 件
- 手動検証：
  - キーボード操作でフォーム入力・ボタン押下が可能であることを確認
  - 200%ズームでもレイアウト崩れなし
